...
export const CSV_TABLE_UI_COPY = ({ csvData }) => {
  ...
  const uniqueTopics = useMemo(
    () => ["All", ...new Set(csvData.map((topic) => topic["Main Topic"]))],
    []
  );
                          ...
                          <TableBody>
                            {mainTopic.Subtopics.map((subtopic, subIndex) =>
                              subtopic.Details.map((detail, detailIndex) => (
                                <TableRow
                                  key={`${subIndex}-${detailIndex}`}
                                  className="hover:bg-accent/5 transition-colors"
                                >
                                  {detailIndex === 0 ? (
                                    <TableCell
                                      className="font-medium align-top"
                                      rowSpan={subtopic.Details.length}
                                    >
                                      {subtopic.Subtopic}
                                    </TableCell>
                                  ) : null}
                                  ...
                                  !detail.Links.some(
                                    (link) =>
                                      link.includes("leetcode") || link.includes("geeksforgeeks")
                                  ) ? (
                                  ...
                                  <TableCell className="text-center align-top">
                                    {detail["Video Link"] &&
                                      detail["Video Link"].includes("https") && (
                                        <a
...
export default CSV_TABLE_UI_COPY;
